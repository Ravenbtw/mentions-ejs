...
app.post('/words', (req, res) => {
  ...
  fs.writeFileSync(
    path.join(__dirname, 'config', 'words.json'),
...
const updateChannelsList = async () => {
  ...
  const follows = await apiClient.users
    .getFollowsPaginated({
      user: user.id,
    })
    .getAll();
  const followedChannels = [
    name,
    ...follows.map(({ followedUserName }) => followedUserName),
  ];
...
setInterval(() => {
  fs.writeFileSync(
    path.join(__dirname, 'config', 'mentions.json'),
    JSON.stringify(mentions, null, 4)
  );
}, 1000 * 60);
